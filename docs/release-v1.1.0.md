...
- 新增 `.github/workflows/flutter-apk.yml`，提供 tag / push 后自动构建 APK 的 CI 示例。
...
本次已补充 GitHub Actions 构建流程。推送代码或 tag 后，CI 会尝试自动执行：
...
- CI：自动构建流水线。代码提交或打 tag 后，云端机器自动下载依赖、检查代码、运行测试、打包 APK，并输出构建产物。
